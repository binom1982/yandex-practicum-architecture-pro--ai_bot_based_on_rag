...
def rerank_docs(query: str, docs, top_k: int = 3, threshold: float = 0.5):
    if not docs:
        return []
...
def format_docs(docs):
    if not docs:
        return "Нет релевантной информации в базе."
    formatted = []
...
ВАЖНО:
- Контекст ниже может быть на английском языке — это нормально. Извлекай факты из него.
- Отвечай на русском, но сохраняй оригинальные термины из базы (Xarn Velgor, Synth Flux).
- Если в Контексте нет информации по вопросу — пиши «Я не знаю».
- Не используй знания извне.
